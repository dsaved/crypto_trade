...
import 'package:crypto_trade/crypto_app/ui_view/trade_tickers_view.dart';
...
class TradeTickerPage extends StatefulWidget {
  const TradeTickerPage({Key key, this.animationController}) : super(key: key);

  final AnimationController animationController;

  @override
  _TradeTickerPageState createState() => _TradeTickerPageState();
}

class _TradeTickerPageState extends State<TradeTickerPage>
  ...
  void initAll() async {
    ...
    String _trades = prefs.getString(Constant.TRADES_PREF_KEY);
    if (_trades != null) {
      trades = json.decode(_trades);
      if (this.mounted) {
        setState(() {});
      }
    }
  ...
  _getData({more = false, silent = false, page = 1, search = ""}) async {
    ...
    await _netUtil
        .post("${Constant.trades}", context, body: params)
        .then((value) {
          setState(() {
            _loading = false;
          });
          ...
          if (value['success'] == true) {
            if (more) {
              trades.insertAll(
                  trades.length, value['trade']);
            } else {
              trades = value['trade'];
              prefs.setString(
                  Constant.TRADES_PREF_KEY, json.encode(trades));
            }
            _loading = false;
            _hasError = false;
            setState(() {});
